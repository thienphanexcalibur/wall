...
export default class PostController extends Controller {
	...
	async addPost(req: Request, res: Response, next: NextFunction): Promise<void> {
		const { title, content }: IPost = res.locals.body;
		const { authId } = res.locals;
		try {
			const newPost = await PostModel.create({ title, content, byUser: authId } as IPost);
			// Append new post into current user
			const foundUser = await UserModel.findById(authId);
